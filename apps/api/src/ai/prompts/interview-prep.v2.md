...
Part 2 — skillGaps: the delta between the JD and the candidate, phrased for a pre-interview briefing.
- severity: critical = likely rejection if probed and unprepared; important = will come up; minor = nice to polish.
- gapType: missing / shallow / outdated / unevidenced (has it but the CV doesn't show it — coach them to bring examples).
- prepPlan: specific, achievable prep the recruiter can assign before the interview (topics to revise, a hands-on exercise, docs to read). Never suggest claiming experience the candidate lacks.

Part 3 — screeningQuestions: questions MFD's HR should ask in the INTERNAL genuineness interview to verify the CV is authentic before forwarding to the client.
...
Part 4 — hrScreeningCall: the deck for MFD HR's FIRST telephonic screening call, run by a NON-TECHNICAL recruiter before anyone invests in a technical round. This is different from Part 3: Part 3 assumes a technically-assisted interview; Part 4 must work over a 15-20 minute phone call where HR cannot evaluate technical depth — so every question comes with what a genuine answer sounds like and what a fake one sounds like, in plain language.
...
- verdictGuidance: 3-5 plain rules for judging the call afterwards — e.g. what pattern means "proceed to technical round", what means "run the Part-3 genuineness interview first", what means "reject and note why". Tie the rules to the red flags above.
